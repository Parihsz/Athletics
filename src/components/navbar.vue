  ...
  <nav class="navbar custom-navbar" role="navigation" aria-label="main navigation">
    ...
    <div class="navbar-menu" :class="{ 'is-active': is_menu_active }">
      <div class="navbar-end">
        <router-link v-if="is_logged_in" to="/league" class="navbar-item nav-link">Leagues</router-link>
        <router-link v-if="is_logged_in" to="/dashboard" class="navbar-item nav-link">Admin</router-link>
        <router-link to="/login" class="navbar-item nav-link">Login</router-link>
      </div>
    </div>
  </nav>
</template>
...
const is_logged_in = computed(() => !!window.user)
